...
Bayesian networks characterize the joint distribution among a set of random 
...
```{r italyselect}
# extract the specified subset from the dataset 
italy <- italy %>%  dplyr::select(c(age, female, cogscore, isced1997_r, bmi))
# rename the education column
italy <- italy %>% rename(education = isced1997_r, obesity = bmi)
# discretize the age as it will be the parent node in the network
italy$age <- cut(italy$age, breaks = c(seq(60,90,5),100), 
                 ordered_result = TRUE, right = FALSE)    
# discretize the cognitive score using 10% (severe) and 40% (mild) quantiles
italy$cogscore <- cut(italy$cogscore, 
                      breaks = c(0, quantile(italy$cogscore, probs = c(.1, .4),
                                             na.rm=TRUE), 29), 
                      ordered_result = TRUE)
# factorize the female  and education columns
italy$female <- factor(italy$female)
italy$education <- factor(italy$education, ordered = TRUE)
italy$obesity <- factor(italy$obesity, ordered = TRUE)
head(italy)
```

```{r italplots, fig.width=8, fig.height=5.5}
# histograms to display the local distributions 
p1 <- ggplot() + geom_bar(aes(x = italy$cogscore),color="darkblue", fill="lightblue")+
  labs(x ="Cogntive impairment")
p2 <- ggplot() + geom_bar(aes(x = italy$age),color="darkblue", fill="lightblue")+
  labs(x ="Age")
p3 <- ggplot() + geom_bar(aes(x = italy$education),color="darkblue", fill="lightblue")+
  labs(x ="Education")
p4 <- ggplot() + geom_bar(aes(x = italy$obesity), color = "darkblue", fill = "lightblue") +
  labs(x = "Obesity")
...
```{r dag}
# initialize the DAG with nodes 
dag = empty.graph(nodes = c("age", "female","cogscore","education", "obesity"))
# construct edge matrix
arc.set = matrix(c("age","cogscore",    # age -> cogscore
                   "female","cogscore",    # female -> cogscore   
                   "education","cogscore",    # education -> cogscore
                   "obesity", "cogscore",    # obesity -> cogscore
                   "age","education",    # age -> education
                   "age", "obesity"),     
                 byrow = TRUE, ncol=2, dimnames =list(NULL,c("from","to")))
# generate DAG with specified edges
arcs(dag) = arc.set
graphviz.plot(dag)
```
...
bn.fit.barchart(bn.bayes$obesity)
...
# distributions of cognitive scores conditional on age, gender and education 
bn.fit.barchart(bn.bayes$cogscore)

```

```{r condplot2, fig.width=9}
# trend of cognitive score in different gender and education groups, as age increases 
p1 = ggplot(mapping = aes(x = rep(seq(1:length(levels(italy$age))),3),
                     y=matrix(t(bn.bayes$cogscore$prob[,,1,1,1]), ncol =1), color = rep(levels(italy$cogscore), each = length(levels(italy$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(italy$age))),
        labels=levels(italy$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Male, Underweight")
p2 = ggplot(mapping = aes(x = rep(seq(1:length(levels(italy$age))),3),
                     y=matrix(t(bn.bayes$cogscore$prob[,,1,1,2]), ncol =1), color = rep(levels(italy$cogscore), each = length(levels(italy$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(italy$age))),
        labels=levels(italy$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Male, Normal")
p3 = ggplot(mapping = aes(x = rep(seq(1:length(levels(italy$age))),3),
                     y=matrix(t(bn.bayes$cogscore$prob[,,1,1,3]), ncol =1), color = rep(levels(italy$cogscore), each = length(levels(italy$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(italy$age))),
        labels=levels(italy$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Male, Overweight")
p4 = ggplot(mapping = aes(x = rep(seq(1:length(levels(italy$age))),3),
                     y=matrix(t(bn.bayes$cogscore$prob[,,1,1,4]), ncol =1), color = rep(levels(italy$cogscore), each = length(levels(italy$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(italy$age))),
        labels=levels(italy$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Male, Obese")
grid.arrange(p1, p2, p3, p4, nrow = 2)
```
...
In the following, we provide an example of using a custom fit, with ordered logistic regression using the [`polr()`](https://www.rdocumentation.org/packages/MASS/versions/7.3-57/topics/polr) function (ordered logistic or probit regression). This helps to reduce the number of parameters and produces a smoother fit as a function of age and education. 

```{r bnfit2, message = FALSE}
library(MASS)
# fit a ordered logistic regression of education over age
olr.fit =polr(education ~ age, data = italy)
olr.fit.2 <- polr(obesity ~ education, data = italy)
# predicted distribution using previous regression
distedu = predict(olr.fit, newdata= data.frame(age=levels(italy$age)),type="p")
distobe = predict(olr.fit.2, 
                  newdata= data.frame(education=levels(italy$education)),type="p")
edu.lv = levels(italy$education)
obe.lv <- levels(italy$obesity)
...
distobe = array(t(distobe), dim=c(length(obe.lv),length(edu.lv)), 
                dimnames = list(obesity = obe.lv, education = edu.lv))
bn.bayes2 = bn.bayes
# update the probability of education with the predicted matrix
bn.bayes2$education = distedu
bn.bayes2$obesity <- distobe
# histograms of new conditional probabilities 
bn.fit.barchart(bn.bayes2$education)
bn.fit.barchart(bn.bayes2$obesity)
```

```{r bnfit2cog, message = FALSE}

olr.fitf =polr(cogscore ~ age + education + obesity, data = italy[italy$female==1,])
olr.fitm =polr(cogscore ~ age + education + obesity, data = italy[italy$female==0,])
edu.lv = levels(italy$education)
age.lv = levels(italy$age)
f.lv = levels(italy$female)
cs.lv = levels(italy$cogscore)
combos = data.frame(age=rep(age.lv,length(edu.lv)), education=rep(edu.lv,each = length(age.lv)), obesity = rep(obe.lv, each = length(edu.lv)*length(age.lv)))
distcsf = predict(olr.fitf, newdata= combos ,type="p")
distcsm = predict(olr.fitm, newdata= combos ,type="p")
distcs = array(dim = c(length(cs.lv), length(age.lv), length(f.lv), length(edu.lv), length(obe.lv)), dimnames = list(cogscore = cs.lv, age = age.lv, female = f.lv, education = edu.lv, obesity = obe.lv))
distcs[,,1,,] = array(t(distcsm), dim=c(length(cs.lv), length(age.lv), 1, length(edu.lv), length(obe.lv)))
distcs[,,2,,] = array(t(distcsf), dim=c(length(cs.lv), length(age.lv), 1, length(edu.lv), length(obe.lv)))
...
p1 = ggplot(mapping = aes(x = rep(seq(1:length(levels(italy$age))),3),
                     y=matrix(t(bn.bayes2$cogscore$prob[,,1,1,1]), ncol =1), color = rep(levels(italy$cogscore), each = length(levels(italy$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(italy$age))),
        labels=levels(italy$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Male, No education, Underweight")
p2 = ggplot(mapping = aes(x = rep(seq(1:length(levels(italy$age))),3),
                     y=matrix(t(bn.bayes2$cogscore$prob[,,1,1,2]), ncol =1), color = rep(levels(italy$cogscore), each = length(levels(italy$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(italy$age))),
        labels=levels(italy$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Male, No education, Normal")
p3 = ggplot(mapping = aes(x = rep(seq(1:length(levels(italy$age))),3),
                     y=matrix(t(bn.bayes2$cogscore$prob[,,1,1,3]), ncol =1), color = rep(levels(italy$cogscore), each = length(levels(italy$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(italy$age))),
        labels=levels(italy$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Male, No education, Overweight")
p4 = ggplot(mapping = aes(x = rep(seq(1:length(levels(italy$age))),3),
                     y=matrix(t(bn.bayes2$cogscore$prob[,,1,1,4]), ncol =1), color = rep(levels(italy$cogscore), each = length(levels(italy$age))))) + 
  geom_point() +
  geom_line() +
  scale_x_discrete(breaks=seq(1:length(levels(italy$age))),
        labels=levels(italy$age)) +
  labs(x = "Age", y= "Conditional probability", color = "Cognitive score", title ="Male, No education, Obese")
grid.arrange(p1, p2, p3, p4, nrow = 2)
```
...
ci.test("cogscore", "obesity", c("education", "age", "female"), test = "mi", data = italy)
```
...
italy.pc <- pc.stable(italy, blacklist = myblacklist, test = "mi")
graphviz.plot(italy.pc)
```
...
```{r grain, message=FALSE, warning=FALSE}
