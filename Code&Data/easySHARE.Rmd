...
```{r}
vars <- c("age", "female", "wave", "country", "mergeid",
          "isced1997_r",    # education
          "partnerinhh", "ch007_hh", "ch007_km", 
          "sp002_mod", "sp008_",    # social isolation 
          "chronic_mod",    # chronic disease
          # "hc002_mod", "hc012_", "hc029_",    # health care
          "eurod", # depression
          "adla", "iadlza", "mobilityind", "lgmuscle", "grossmotor", 
          "finemotor", "br015_",    # physical activity
          "bmi2",    # obesity
...
w5_com <- w5_com %>% 
  rename(id = mergeid,
         ...
         income = co007_,
         depression = eurod) %>% 
  mutate_at(c("ch007_hh", "ch007_km", "sp002_mod", "sp008_", 
              "smoking", "ever_smoked"), ~if_else(.x == 5, 0, 1)) %>%
  mutate(smoke = smoking + ever_smoked,
         physical = adla + iadlza + mobilityind + lgmuscle + grossmotor + 
           finemotor + br015_,
         social = ch007_hh + ch007_km + sp002_mod + sp008_,
         cogscore = recall_1 + recall_2 + numeracy_2) %>% 
  dplyr::select(c(age, id, female, education, alcohol, obesity, chronic, work, 
                  income, depression, smoke, physical, social,
                  cogscore, country))

w5_work_97_id <- w5_com$id[w5_com$work == 97]
w5_edu_9597_id <- w5_com$id[w5_com$education > 90]
w5_wrong_id <- union(w5_work_97_id, w5_edu_9597_id)
w5_com <- w5_com[w5_com$id %in% w5_wrong_id == FALSE, ]

w5_com$age <- cut(w5_com$age, breaks = c(seq(60,90,5),100), 
                  ordered_result = TRUE, right = FALSE)
w5_com$cogscore <- cut(w5_com$cogscore, 
                       breaks = c(0, quantile(w5_com$cogscore, 
                                              probs = c(0.1, 0.4)), 29), 
                       ordered_result = TRUE)
w5_com$female <- factor(w5_com$female)
w5_com$education <- factor(w5_com$education, ordered = TRUE)
w5_com$alcohol <- factor(w5_com$alcohol, ordered = TRUE)
w5_com$obesity <- factor(w5_com$obesity, ordered = TRUE)
w5_com$chronic <- cut(w5_com$chronic, breaks = c(0, 1, 3, max(w5_com$chronic)),
                       ordered_result = TRUE, right = FALSE)
w5_com$work <- factor(w5_com$work, ordered = TRUE)
w5_com$income <- factor(w5_com$income, ordered = TRUE)
w5_com$depression <- cut(w5_com$depression, breaks = c(0, 4, 8, 12),
                         ordered_result = TRUE, right = FALSE)
w5_com$smoke <- factor(w5_com$smoke, ordered = TRUE)
w5_com$physical <- cut(w5_com$physical, breaks = c(0, 2, 5, 29), 
                       ordered_result = TRUE, right = FALSE)
w5_com$social <- factor(w5_com$social, ordered = TRUE)
```

```{r}
sort(table(w5_com$country), decreasing = TRUE)
country_high <- c(31, 20, 18, 13, 14, 11, 23, 12, 17)
w5_com_high <- w5_com[w5_com$country %in% country_high, ]
w5_com_low <- w5_com[setdiff(row.names(w5_com), row.names(w5_com_high)),]
```

```{r}
w5_12 <- w5_com[w5_com$country == 12, ]
head(w5_12)
```
...
dag <- empty.graph(nodes = c("age", "female", "cogscore", "education", 
                             "obesity", "alcohol", "smoke", "work", "income",
                             "depression", "social"))
arc.set <- matrix(c("age", "education",
                    "age", "physical",
                    "age", "cogscore",
                    "female", "education",
                    "female", "work", 
...
```{r}
# using all complete data in wave 5
w5_com.iamb <- iamb(subset(drop_na(w5_com), select = -c(id, country)), 
                   blacklist = myblacklist, test = "mi")
graphviz.plot(w5_com.iamb)

w5_com.pc <- pc.stable(subset(drop_na(w5_com), select = -c(id, country)), 
                      blacklist = myblacklist, test = "mi")
graphviz.plot(w5_com.pc)
```

```{r}
# using complete data in high income country in wave 5
w5_com_high.iamb <- iamb(subset(drop_na(w5_com_high), select = -c(id, country)), 
                   blacklist = myblacklist, test = "mi")
graphviz.plot(w5_com_high.iamb)

w5_com_high.pc <- pc.stable(subset(drop_na(w5_com_high), 
                                   select = -c(id, country)), 
                      blacklist = myblacklist, test = "mi")
graphviz.plot(w5_com_high.pc)
```

```{r}
# using complete data in low income country in wave 5
w5_com_low.iamb <- iamb(subset(drop_na(w5_com_low), select = -c(id, country)), 
                   blacklist = myblacklist, test = "mi")
graphviz.plot(w5_com_low.iamb)

w5_com_low.pc <- pc.stable(subset(drop_na(w5_com_low), select = -c(id, country)), 
                      blacklist = myblacklist, test = "mi")
graphviz.plot(w5_com_low.pc)
```

```{r}
w5_12.iamb <- iamb(subset(drop_na(w5_12), select = -c(id, country)), 
                   blacklist = myblacklist, test = "mi")
graphviz.plot(w5_12.iamb)

w5_12.pc <- pc.stable(subset(drop_na(w5_12), select = -c(id, country)), 
                      blacklist = myblacklist, test = "mi")
graphviz.plot(w5_12.pc)
```
