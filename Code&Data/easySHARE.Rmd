...
w5_.hc.mod <- drop.arc(w5_.hc.mod, from = "income", to = "smoking")
w5_.hc.mod <- drop.arc(w5_.hc.mod, from = "depression", to = "physical")
w5_.hc.mod <- drop.arc(w5_.hc.mod, from = "education", to = "alcohol")
...
w5_.hc.mod <- set.arc(w5_.hc.mod, from = "physical", to = "obesity")
w5_.hc.mod <- set.arc(w5_.hc.mod, from = "alcohol", to = "obesity")
...
# modify some unreasonable arrows
w5_.iamb.mod <- drop.arc(w5_.iamb.mod, from = "alcohol", to = "income")
w5_.iamb.mod <- drop.arc(w5_.iamb.mod, from = "physical", to = "depression")
w5_.iamb.mod <- drop.arc(w5_.iamb.mod, from = "chronic", to = "depression")
w5_.iamb.mod <- drop.arc(w5_.iamb.mod, from = "education", to = "alcohol")
w5_.iamb.mod <- reverse.arc(w5_.iamb.mod, from = "chronic", to = "obesity")
w5_.iamb.mod <- reverse.arc(w5_.iamb.mod, from = "physical", to = "chronic")

# test if country has direct effect on cognitive score
ci.test("country", "cogscore", 
        c("age", "female", "education", "physical", "depression"),
        data = w5_[rf_score], test = "mi")
...
ci.test("alcohol", "cogscore", 
        c("age", "female", "country", "education", "physical", "depression"),
        data = w5_[rf_score], test = "mi")

# test if drinking behaviour has influence on obesity
ci.test("alcohol", "obesity", c("female", "chronic", "smoking"),
        data = w5_[rf_score], test = "mi")
w5_.iamb.mod <- set.arc(w5_.iamb.mod, from = "alcohol", to = "obesity")

# test if physical activity frequency has influence on obesity
ci.test("physical", "obesity", 
        c("female", "smoking", "alcohol", "chronic"),
        data = w5_[rf_score], test = "mi")
w5_.iamb.mod <- set.arc(w5_.iamb.mod, from = "physical", to = "obesity")
...
ci.test("obesity", "depression", 
        c("female", "chronic", "income", "social"),
        data = w5_[rf_score], test = "mi")
...
ci.test("age", "physical", data = w5_[rf_score], test = "mi")
w5_.iamb.mod <- set.arc(w5_.iamb.mod, from = "age", to = "physical")
```
```{r}
...
# as age increases (holding education and depression)
...
```
```{r}
olr.fitf <- polr(cogscore ~ age + country + education + depression,
                 data = w5_[w5_$female== 1, ])
olr.fitm <- polr(cogscore ~ age + country + education + depression,
                 data = w5_[w5_$female== 0, ])

combos <- data.frame(age = rep(a.lv, length(c.lv)), 
                     country = rep(c.lv, each = length(a.lv)),
                     education = rep(e.lv, each = length(a.lv) * length(c.lv)),
                     depression = rep(d.lv, each = length(a.lv) * 
                                        length(c.lv) * length(e.lv)))
